...
import { UserCheck, Users, MessageSquare, Mail, Phone, Calendar } from "lucide-react";
...
export function MentorDetailsSheet({ mentor, open, onOpenChange }: MentorDetailsSheetProps) {
      ...
      <SheetContent className="sm:max-w-[600px] w-full">
        ...
        <Tabs defaultValue="profile" className="mt-6">
            ...
            <Card className="p-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm">{mentor.phone || t('common.notProvided')}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Calendar className="w-4 h-4 text-muted-foreground" />
                  <span className="text-sm">{mentor.joinedDate || t('common.notProvided')}</span>
                </div>
              </div>
            </Card>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <Badge variant={mentor.status === "active" ? "default" : "secondary"}>
                      {t(`common.${mentor.status}`)}
                    </Badge>
                  </div>
                </div>
                <div>
                  <span className="text-sm text-muted-foreground">{t('common.assignedGroups')}</span>
                  <p className="font-medium">{mentor.groups} {t('common.groups')}</p>
                </div>
              </div>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="groups" className="space-y-4 mt-4">
            <div className="space-y-3">
              {Array.from({ length: mentor.groups }, (_, i) => i + 1).map((group) => (
                <Card key={group} className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-medium">Mentoring Group {group}</h4>
                ...
                <Card key={request} className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-medium">Support Request #{request}</h4>
                        <p className="text-sm text-muted-foreground">Student assistance needed</p>
                      </div>
                    </div>
